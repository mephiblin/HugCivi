...
- The library grid requests 50-card pages with `/api/library?limit=50&page=N&mode=...&path=...`; sort values include `az`, `za`, `date_desc`, `date_asc`, and `favorite`, with legacy `date` kept as a newest-first alias. Legacy `/api/library` array behavior remains available when `limit` and `page` are omitted.
- Selected-folder live library pagination sorts a bounded stable scan window before slicing pages. The default small-folder window is three 50-card pages, keeping 100-card folders responsive while avoiding the earlier first/second-page repeat. The UI clears stale cards into the loading state on page/sort changes so CJK-titled cards do not appear to linger across pages.
...
- `/api/folders` stays a bounded initial tree for sidebar compatibility and now uses `initial_folder_tree()` with direct root children only (`FOLDER_TREE_INITIAL_MAX_DEPTH=1`). `/api/folders/children` loads direct child folder rows on demand with `limit`/`cursor` pagination (default 200, max 500), and the current search/move UI operates over the loaded tree plus lazy expansions. Hitomi gallery archive folders are treated as leaf nodes, so Tree navigation does not scan page files inside each downloaded gallery. Server-side folder search and optional folder indexing remain the follow-up direction in `docs/folder-tree-scaling-design-2026-07-06.md`.
...
- Copy-only transfer is registered-target-only outbound work. HugCivi supports `rclone` targets backed by `/config/rclone/rclone.conf` and `receiver` targets backed by the sibling `/home/inri/문서/HugCivi-Receiver` app. Receiver targets can browse the mounted PC `/receive` tree through `/api/transfer/targets/{target_id}/receiver/tree`; the browser never receives the Receiver token and sends only `target_id`, `/data` `source_path`, and optional `destination_subpath`.
- `/data_remote` connected transfer is a proposal, not current behavior. The design is in `docs/data-remote-transfer-design-2026-07-06.md`; it treats one or more host-mounted PC/Synology/remote folders as copy-only `local_mount` transfer targets while keeping `/data_remote` separate from the indexed `/data` library.
