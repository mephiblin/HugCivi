...
- Selected-folder live library pagination now reports known `total_count`/`total_pages` when the selected-folder scan completes within the internal path budget, so ordinary folders show their full page list. Completed selected-folder scans are kept in a short in-memory page cache so page/sort changes do not rescan the same NAS folder on every click. If a scan cannot complete within that budget, the API leaves totals unknown and the UI keeps the previous/current/next fallback. The UI clears stale cards into the loading state on page/sort changes so CJK-titled cards do not appear to linger across pages.
