...
- Bare `pawchive.pw` and `pawchive.st` HTTP(S) URLs, including their `www` hosts, route directly to the gallery-dl provider. Keep this exact-host match ahead of generic HTTP routing; other Pawchive subdomains and lookalike domains stay generic unless explicitly prefixed with `gallery-dl`/`gdl`. Pawchive uses the Kemono-style post structure: after gallery-dl writes `info.json`, Patreon Vimeo embeds are followed with yt-dlp using the Patreon Referer and saved in the same archive.
