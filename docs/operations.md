...
- The browser coalesces non-blocking library card renders into the next animation frame, updates favorite button state in place when sorting does not need to change, and uses card-level CSS containment so offscreen cards cost less layout/paint work.
