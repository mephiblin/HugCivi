...
| Settings and secrets | Settings modal, `POST /settings` | `app/main.py::save_settings`, `app/db.py::settings_status`, `db.set_setting`, `db.get_setting` | settings panes in `app/templates/index.html` | `settings` table, env fallback, `/config/startup.env` for gallery-dl auto-update; saved tokens, cookies, proxy URLs, and extra options are treated as secrets | `test_settings_status_never_returns_secret_values`, `test_startup_config_writer_persists_gallery_dl_update_toggle` |
