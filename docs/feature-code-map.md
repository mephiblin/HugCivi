...
| Internal job scheduler | ZIP/transcode/poster/thumbnail backfill/transfer/library reindex work | `app/internal_jobs.py`, `register_internal_job_handlers`, `run_archive_zip_job`, `run_media_transcode_job`, `run_media_poster_job`, `run_media_thumbnail_backfill_job`, `run_library_reindex_job`, `run_transfer_copy_job` | UI polls job status and artifact URLs; library refresh queues `library_reindex`; library thumbnail button queues `media_thumbnail_backfill`; transfer modal queues `transfer_copy` | same `jobs` table with non-download `job_kind`; `library_reindex` stores scope/progress metadata, reuses active duplicate scopes, bulk-upserts scan batches, and uses `library_scan_state` scoped cursors; `media_thumbnail_backfill` stores candidate counts and results in metadata; `transfer_copy` stores the request snapshot, SQLite job log, and manifest artifact refs | `test_internal_job_actions_use_internal_queue`, `test_internal_job_rows_are_separate_from_download_resume_list`, `test_library_reindex_scope_filters_source_without_clearing_unrelated_rows`, `test_library_reindex_job_resumes_scoped_batches`, `test_media_thumbnail_backfill_job_uses_card_representatives`, transfer internal job tests in `tests/test_transfer_api.py` |
