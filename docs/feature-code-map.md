...
| yt-dlp/YouTube and preferred video sites | YouTube URL, `yt`, `youtube`, `yt-dlp`, `ytdl:` | `maybe_parse_youtube_cli`, `parse_ytdlp_url`, `download_gallerydl`, `yt_dlp_command`, `app/ytdlp_sites.py` | media archive display, subtitles, settings modal `YouTube/yt-dlp Proxy` | `/data/gallery-dl/youtube.com/playlist/<id>` or `/data/gallery-dl/youtube.com/channel/<name>` for YouTube, subtitle files, ytdlp config args, `YT_DLP_PROXY` env/UI setting | `tests/test_youtube_parser.py`, `tests/test_main_urls.py`, downloader runtime yt-dlp tests |
