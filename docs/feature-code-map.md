...
| Settings and credentials | Settings modal, `POST /settings` | `app/main.py::save_settings`, `app/db.py::settings_status`, `db.set_setting`, `db.get_setting` | settings panes in `app/templates/index.html`; credential values render in plaintext in the authenticated editor | `settings` table, env fallback, `/config/startup.env` for gallery-dl auto-update; saved tokens, cookies, proxy URLs, and extra options are credential-bearing settings | `test_settings_status_returns_credential_values_for_settings_form`, `test_settings_post_updates_runtime_auth_values`, `test_startup_config_writer_persists_gallery_dl_update_toggle` |
...
| Job list and controls | `/api/jobs`, `/api/jobs/{id}`, pause/resume/retry/delete/clear, `/jobs/{id}/log`, row `이동` jump to target folder | `api_jobs`, `api_job`, `api_pause_job`, `api_resume_job`, `api_retry_job`, `api_delete_job`, `job_log`, `decorate_job` | `refreshJobs`, `renderJobs`, `renderMobileJobs`, `handleJobAction`, `goToJobFolder`, `clearJobHistory` | `jobs.status`, `jobs.log`, `jobs.target_dir`/decorated `target_path`, partial cleanup on history clear | `test_job_list_payload_omits_log_but_detail_and_log_endpoint_keep_it`, `test_job_summary_query_omits_heavy_fields_and_supports_cursor`, `test_retry_failed_job_requeues_existing_job`, `test_clear_history_removes_failed_partial_files_before_deleting_rows`, `test_home_template_declares_storage_folder_search_ui` |
...
| Civitai image archive and resource jobs | Civitai image page URL, media viewer generation panel, model-version resource health | `download_civitai_image_page`, `normalize_civitai_image_record`, `create_civitai_image_resource_jobs`, `civitai_existing_resource_state`, `api_civitai_resource_health` | `normalizeCivitaiImageMetadata`, `renderGenerationPanel`, `generationModelVersionIds`, `checkCivitaiResourceHealth` | `_civitai_image_metadata.json`, child Civitai jobs, health from jobs/sidecars | `tests/test_civitai_viewer_health.py`, downloader runtime Civitai resource tests |
...
| ASMR.one work downloads | ASMR.one `/work/RJ...` and `/work/<id>/DLSITE/RJ...` URLs; normal external queue under the `asmrone` provider bucket | `parse_asmrone_url`, `download_asmrone`, `asmrone_manifest_entries`, `asmrone_download_action_url`, `provider_key_for_parsed`; file bodies use track `mediaDownloadUrl` plus `action=download`, not `mediaStreamUrl` | job list/library display and audio media viewer support in `index.html`/`style.css` | `/data/asmr.one/...`, Unicode-safe track paths, downloaded track files, `_asmrone_metadata.json`, redacted `_asmrone_tracks.json`, `_asmrone_manifest.json` with per-file `download_status` and `failed_file_count`, `_archive_metadata.json`; partial file failures are preserved when at least one file downloads | `tests/test_asmrone_provider.py` |
...
| Library view and index | Library tab/cards, `/api/library`, `/api/library?mode=live&path=...`, `/api/library/reindex` | `api_library`, `library_items`, `live_library_items`, `iter_data_paths`, `start_library_indexer`, `scan_library_index_batch`, `library_item_for_path`, yt-dlp `.info.json` title helpers, Civitai sidecar restore helpers, DB library index helpers, `api_clear_jobs` index reset | `renderLibrary`, `libraryCards`, `refreshLibraryItems`, sorting/favorites/source URL actions; folder navigation requests live path scans | `library_items`, `library_scan_state`, sidecar metadata from `/data`, yt-dlp info sidecars; clearing job history resets stale index rows and live scans restore filesystem-backed cards | `test_library_index_scan_populates_db_backed_library_items`, `test_library_items_restore_filesystem_card_after_job_row_deleted`, `test_library_live_path_finds_child_model_cards_when_index_is_stale`, `test_clear_history_resets_stale_library_index_so_existing_model_cards_survive`, `test_library_items_index_generic_sidecar_folder`, `test_library_items_use_ytdlp_info_title_for_single_video_archive`, `test_decorated_gallerydl_job_uses_ytdlp_info_title_for_single_video` |
...
| Filesystem operations | Context menu create folder/rename/move/delete/properties/preview/download; selected-folder scoped folder search; move destination tree picker; manual folder-tree refresh | `api_create_folder`, `api_rename_path`, `api_move_path`, `api_delete_path`, `api_path_properties`, `existing_data_path`, `ensure_mutable_path`, `ensure_no_active_jobs` | `selectFolderPath`, folder search/create/move modal helpers, `showContextMenu`, `handleContextAction`, `postFileAction`, `refreshFolders`, `showProperties`; file actions update tree/library in place and keep the active folder when possible | `/data` filesystem, DB path prefix updates, favorites/notes/index maintenance | `test_home_template_declares_storage_folder_search_ui`, `test_api_create_folder_creates_child_and_rejects_nested_name`, `test_safe_join_and_relative_path_preserve_internal_symlink_itself`, `test_active_job_protection_includes_jobs_without_target_dir`, `test_prefix_clears_escape_like_wildcards` |
...
| Media viewer | Library media card, `/api/media/list`, file/play/poster/subtitle APIs | image/video/audio listing, direct audio playback, video subtitle/transcode/poster helpers in `app/main.py` | `openMediaViewer`, `renderMediaViewer`, `setupMediaPlayer`, `prepareMediaPlayback`, `prepareMediaPoster` | media files in `/data`; video transcode/poster cache in `/config/media-cache` | `test_uncached_video_payload_requires_async_media_jobs`, media transcode/poster artifact tests, subtitle tests, `test_asmrone_audio_files_are_media_items` |
...
| Settings/folders | `POST /settings`, legacy `POST /folders`, `GET/POST /api/folders` | `save_settings`, `create_folder`, `api_folders`, `api_create_folder` |
