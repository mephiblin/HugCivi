...
| Job list and controls | `/api/jobs`, `/api/jobs?limit=50&page=N`, `/api/jobs/{id}`, pause/resume/retry/delete/clear, `/jobs/{id}/log`, row `이동` jump to target folder | `api_jobs`, `jobs_page_payload`, `api_job`, `api_pause_job`, `api_resume_job`, `api_retry_job`, `api_delete_job`, `job_log`, `decorate_job`, `db.count_jobs`, `db.list_job_summaries` | `refreshJobs`, `renderJobs`, `renderJobsPagination`, `renderMobileJobs`, `handleJobAction`, `goToJobFolder`, `clearJobHistory` | `jobs.status`, `jobs.log`, `jobs.target_dir`/decorated `target_path`, 50-row numbered page state, partial cleanup on history clear | `test_job_list_payload_omits_log_but_detail_and_log_endpoint_keep_it`, `test_job_summary_query_omits_heavy_fields_and_supports_cursor`, `test_retry_failed_job_requeues_existing_job`, `test_clear_history_removes_failed_partial_files_before_deleting_rows`, `test_home_template_declares_storage_folder_search_ui` |
