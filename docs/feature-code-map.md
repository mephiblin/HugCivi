...
| Library view and index | Library tab/cards, `/api/library`, `/api/library/reindex` | `library_items`, `start_library_indexer`, `scan_library_index_batch`, `library_item_for_path`, yt-dlp `.info.json` title helpers, DB library index helpers | `renderLibrary`, `libraryCards`, sorting/favorites/source URL actions | `library_items`, `library_scan_state`, sidecar metadata from `/data`, yt-dlp info sidecars | `test_library_index_scan_populates_db_backed_library_items`, `test_library_items_restore_filesystem_card_after_job_row_deleted`, `test_library_items_index_generic_sidecar_folder`, `test_library_items_use_ytdlp_info_title_for_single_video_archive`, `test_decorated_gallerydl_job_uses_ytdlp_info_title_for_single_video` |
...
| Filesystem operations | Context menu create folder/rename/move/delete/properties/preview/download; selected-folder scoped folder search; move destination tree picker | `api_create_folder`, `api_rename_path`, `api_move_path`, `api_delete_path`, `api_path_properties`, `existing_data_path`, `ensure_mutable_path`, `ensure_no_active_jobs` | `selectFolderPath`, folder search/create/move modal helpers, `showContextMenu`, `handleContextAction`, `postFileAction`, `showProperties` | `/data` filesystem, DB path prefix updates, favorites/notes/index maintenance | `test_home_template_declares_storage_folder_search_ui`, `test_api_create_folder_creates_child_and_rejects_nested_name`, `test_safe_join_and_relative_path_preserve_internal_symlink_itself`, `test_active_job_protection_includes_jobs_without_target_dir`, `test_prefix_clears_escape_like_wildcards` |
...
| `tests/test_asmrone_provider.py` | ASMR.one URL parsing, source URL display, mediaDownloadUrl body download handling, sidecar metadata, and audio media recognition. |
