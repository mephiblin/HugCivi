...
| Internal job scheduler | ZIP/transcode/poster/thumbnail backfill work | `app/internal_jobs.py`, `register_internal_job_handlers`, `run_archive_zip_job`, `run_media_transcode_job`, `run_media_poster_job`, `run_media_thumbnail_backfill_job` | UI polls job status and artifact URLs; library thumbnail button queues `media_thumbnail_backfill` | same `jobs` table with non-download `job_kind`; `media_thumbnail_backfill` stores candidate counts and results in metadata | `test_internal_job_actions_use_internal_queue`, `test_internal_job_rows_are_separate_from_download_resume_list`, `test_media_thumbnail_backfill_job_uses_card_representatives` |
