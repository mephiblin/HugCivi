...
| `app/transfer.py` | Copy-only transfer validation, local mount path/copy safety, rclone and HugCivi Receiver destination handling, rclone command building, policy sanitization, and output redaction helpers. |
...
| Copy-only transfer | Context menu `전송`, settings pane target management for `local_mount`, HugCivi Receiver, or rclone targets, settings-pane `/data` root clone for local mount targets, registered target APIs, local mount and Receiver folder tree routes, `POST /api/transfer/preflight`, `POST /api/transfer/jobs`, `POST /api/transfer/data-root/preflight`, `POST /api/transfer/data-root/jobs` | Transfer API/routes/runner in `app/main.py`, transfer target persistence in `app/db.py`, local mount/rclone/Receiver helpers in `app/transfer.py`, `transfer_copy` handler registered with `app/internal_jobs.py` | `openTransferModal`, `loadTransferTargets`, `refreshTransferReceiverBrowser`, `refreshTransferPreflight`, `submitTransferJob`, `loadTransferSettingTargets`, `renderTransferDataRootTargets`, `refreshTransferDataRootPreflight`, `submitTransferDataRootClone`, `saveTransferSettingTarget`; settings UI shows local mount creation first, `/data` clone separately, registered targets separately, and keeps Receiver/rclone fields in an advanced disclosure; local mount and Receiver modal flows submit only `target_id`, `/data` `source_path`, and selected `destination_subpath`; `/data` root clone sends no `source_path` and is local mount only; rclone keeps the default target base path | `transfer_targets` table with `kind`, local mount relative paths, rclone remote fields, Receiver URL/token fields, and copy policy; `/data_remote` optional connected-folder destination root; local mount tree payloads and Receiver proxy payloads avoid secrets/absolute host paths; data-root clone job payloads carry only target ID, destination subpath, and an internal clone flag; `jobs` rows with `job_kind='transfer_copy'` and `source='transfer'`; `/config/rclone/rclone.conf` for rclone credentials; manifest artifacts under `/config/transfer-manifests`; SQLite job logs | `tests/test_transfer_api.py`, `tests/test_transfer_core.py`, `tests/test_transfer_db.py`; template checks for context menu, modal fields, split settings pane, local mount/Receiver fields, token/path hiding, tree routes, data-root clone controls, and payload shape |
...
| Transfer | `GET/POST /api/transfer/targets`, `PATCH/DELETE /api/transfer/targets/{target_id}`, `GET /api/transfer/targets/{target_id}/local-mount/tree`, `GET /api/transfer/targets/{target_id}/receiver/tree`, `POST /api/transfer/preflight`, `POST /api/transfer/jobs`, `POST /api/transfer/data-root/preflight`, `POST /api/transfer/data-root/jobs` | copy-only transfer target validation, local mount/Receiver tree browsing, per-source and `/data` root clone preflight, and `transfer_copy` internal job creation |
...
| `tests/test_transfer_api.py` | Transfer API target CRUD, local mount and Receiver tree browsing, copy-only API validation, source/path safety, internal job creation, manifest writing, and transfer template affordances. |
| `tests/test_transfer_core.py` | local mount path/tree/copy safety, rclone remote/path validation, copy/copyto argv construction, env defaults, policy clamping, and sync/move/delete rejection. |
