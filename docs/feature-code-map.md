...
| ASMR.one work downloads | ASMR.one `/work/RJ...` and `/work/<id>/DLSITE/RJ...` URLs | `parse_asmrone_url`, `download_asmrone`, `asmrone_manifest_entries`, `asmrone_download_action_url` | job list/library display and audio media viewer support in `index.html`/`style.css` | `/data/asmr.one/...`, audio files, `_asmrone_metadata.json`, `_asmrone_tracks.json`, `_asmrone_manifest.json`, `_archive_metadata.json` | `tests/test_asmrone_provider.py` |
...
| `tests/test_asmrone_provider.py` | ASMR.one URL parsing, source URL display, media body download URL handling, sidecar metadata, and audio media recognition. |
