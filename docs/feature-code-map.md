...
| `app/transfer.py` | Copy-only rclone transfer validation, destination resolution, command building, policy sanitization, and output redaction helpers. |
...
| Copy-only transfer | Context menu `전송`, settings pane target management, registered target APIs, `POST /api/transfer/preflight`, `POST /api/transfer/jobs` | Transfer API/routes/runner in `app/main.py`, transfer target persistence in `app/db.py`, copy command helpers in `app/transfer.py`, `transfer_copy` handler registered with `app/internal_jobs.py` | `openTransferModal`, `loadTransferTargets`, `refreshTransferPreflight`, `submitTransferJob`, `loadTransferSettingTargets`, `saveTransferSettingTarget`; modal submits only `target_id` and `/data` `source_path` | `transfer_targets` table, `jobs` rows with `job_kind='transfer_copy'` and `source='transfer'`, `/config/rclone/rclone.conf`, manifest artifacts under `/config/transfer-manifests`, SQLite job logs | `tests/test_transfer_api.py`, `tests/test_transfer_core.py`, `tests/test_transfer_db.py`; template checks for context menu, modal fields, settings pane, and payload shape |
...
| `/config/rclone/rclone.conf` | transfer config | Operator-managed rclone remote definitions. Remote credentials stay in this file, not in transfer target rows. |
