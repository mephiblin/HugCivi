...
| Library view and index | Library tab/cards, legacy array `/api/library`, paged `/api/library?limit=50&page=N&sort=...` with A-Z/Z-A/newest/oldest/favorite sort values, optional `source_group`/`category` filters, DB-first selected-folder index pages, explicit `/api/library?mode=live&path=...`, scoped `/api/library/reindex?path=...&source_group=...`, selected-folder `POST /api/media/thumbnail-jobs` | `api_library`, `api_library_reindex`, `library_items_page_payload`, `library_items`, `live_library_items`, `live_library_items_with_completion`, `cached_selected_folder_live_items`, `clear_live_library_page_cache`, `normalize_library_source_group`, `library_index_fields_for_item`, `iter_data_paths`, `iter_data_paths_with_completion`, `start_library_indexer`, `scan_library_index_batch`, `library_item_for_path`, card thumbnail helpers, `thumbnail_ready_for_url`, `thumbnail_backfill_candidates`, `api_media_thumbnail_jobs`, yt-dlp `.info.json` title helpers, Civitai sidecar restore helpers, DB library index/search helpers, `api_clear_jobs` index reset | `renderLibrary`, `renderLibraryPagination`, `refreshLibraryItems`, `refreshLibraryForActivePath`, `refreshLibraryIndexScope`, `queueLibraryThumbnailBackfill`, `thumbnailImageReady`, `thumbnailReadyAttr`, `libraryJobSignature`, sorting/source-group/favorites/source URL actions; folder navigation requests 50-card DB-backed pages by default, uses `source_group` filters, shows full page numbers when totals are known, and falls back to explicit/automatic live mode when indexed rows are not available; card thumbnail requests are viewport-based, use `thumbnail_ready` to allow up to 10 cached `/api/media/thumbnail` fetches, and keep cold/generating fetches capped at 3; selected folders can queue representative-thumbnail backfill jobs | `library_items` with `source`, `source_group`, `model_category`, `parent_path`, `sort_title`, `library_scan_state`, selected-folder live page cache, sidecar metadata from `/data`, yt-dlp info sidecars; clearing job history resets stale index rows and live scans restore filesystem-backed cards; completed selected-folder live scans return exact `total_count`/`total_pages` and reuse cached items for page navigation until invalidated or expired; first visits and existing filesystem cards request thumbnails only as cards enter the viewport, so large pages do not dispatch 100 uncached thumbnail generations at once; card thumbnail URLs include a file-version query token and cache-hit responses use long-lived private immutable `Cache-Control`; selected-folder thumbnail backfill scans card representatives only and writes disposable JPEGs under `/config/media-cache/thumbnails` | `test_library_api_keeps_legacy_array_and_returns_paged_wrapper`, `test_library_api_date_sort_supports_newest_oldest_and_legacy_alias`, `test_library_index_schema_stores_source_group_fields`, `test_library_source_group_classification`, `test_library_api_selected_folder_uses_index_before_live_scan`, `test_library_reindex_scope_filters_source_without_clearing_unrelated_rows`, `test_selected_folder_live_library_reports_total_pages_beyond_stable_scan_window`, `test_selected_folder_live_library_five_page_boundaries_do_not_backfill_cards`, `test_selected_folder_live_library_reuses_completed_scan_for_page_navigation`, `test_selected_folder_live_library_keeps_unknown_totals_when_scan_is_incomplete`, `test_live_library_pagination_sorts_stable_scan_window`, `test_library_index_scan_populates_db_backed_library_items`, `test_library_items_restore_filesystem_card_after_job_row_deleted`, `test_library_live_path_finds_child_model_cards_when_index_is_stale`, `test_clear_history_resets_stale_library_index_so_existing_model_cards_survive`, `test_media_thumbnail_endpoint_creates_and_reuses_cached_image`, `test_library_item_marks_ready_card_thumbnail_cache`, `test_media_thumbnail_endpoint_rejects_root_and_symlink`, `test_media_thumbnail_backfill_job_uses_card_representatives` |
