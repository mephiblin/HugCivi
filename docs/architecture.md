...
- ComfyUI settings targets can store optional `policy.comfyui_mappings` from fixed HugCivi `stable-diffusion/<route>` prefixes to target destination subfolders; transfer preflight/jobs apply those mappings only when the request does not already provide a `destination_subpath`, Civitai resource transfer falls back to the standard mappings when a target is clearly rooted at `ComfyUI/models` or uses a device-prefixed models-share basename such as `5060TI_ComfyUI-models`, and Civitai model archives keep their model/version folder context when those mapped destinations are built
...
- the browser's bottom-right transfer queue polls `/api/jobs?source=transfer` independently of the main jobs filter, shows active-first/recent transfer rows and progress, and can jump to the full Transfer-filtered jobs table; decorated `transfer_copy` summaries add source path, destination subpath, and root-clone display fields from the sanitized internal payload without exposing target credentials
