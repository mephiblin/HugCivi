...
- 검색은 현재 로드된 `.folder-picker[data-path]` 항목을 대상으로 하되, 선택된 폴더가 있으면 그 폴더와 하위 폴더로 범위를 제한한다.
- 선택 폴더가 없거나 `/data` 루트를 선택한 상태에서는 현재 로드된 트리 전체를 검색한다.
...
### 1차 구현: 선택 폴더 범위 안의 로드된 폴더 트리 검색
...
- 빠르고 위험도가 낮다.
- 선택된 tree 폴더가 있으면 그 폴더와 하위 경로만 보여주므로, 큰 트리에서 관련 없는 결과를 줄일 수 있다.
...
```text
/data 전체에서 3개 결과
/data/stable-diffusion 내부에서 2개 결과
```

또는 결과가 없을 때:

```text
/data/stable-diffusion 내부에서 결과가 없습니다.
```
